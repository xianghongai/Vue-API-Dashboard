// ==UserScript==
// @name:zh-CN   Vue API 看板 (^2.0.0)
// @name         Vue API Dashboard (^2.0.0)
// @namespace    https://github.com/xianghongai/Vue-API-Dashboard
// @version      0.0.1
// @description:zh-CN  更方便的查看 Vue API
// @description  Better view for Vue API
// @author       Nicholas Hsiang / 山茶树和葡萄树
// @icon         https://xinlu.ink/favicon.ico
// @match        https://cn.vuejs.org/v2/api/*
// @match        https://vuejs.org/v2/api/*
// @grant        none
// ==/UserScript==
(() => {
  "use strict";

  const titleText = "Vue API Dashboard";

  const gridSelector = ".sidebar-inner .menu-root";
  const girdIsList = false; // 如果提取的是一个 Node 数组
  const columnSelector = ".menu-root>li";
  const columnTitleSelector = ".menu-root>li>a.section-link";
  const menuListSelector = ".menu-sub";
  const menuItemSelector = ".menu-sub li";
  ...
  const compactColumnLimit = 13; // 多列数据组合上限
